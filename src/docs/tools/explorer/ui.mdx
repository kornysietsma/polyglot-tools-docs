...
The centre of the display shows the files in your project - see [Voronoi Treemap](/voronoi-treemap) for an explanation.  The colouring depends on the currently chosen visualization.
...
"Metric for comparing users and teams" controls how users and teams are compared - TODO: link to the relevant metrics pages when I've written them!

"Code Server" - see the [Simple Code Server](/tools/simple-code-server/) page for more on the code server

"Remote url template" - this allows you to change how links to external code are generated.  The default is `https://{host}/{path}/{project}/blob/{ref}/{file}` - this works for github, if your code is on a different site you will need to change this field.  TODO: this needs better docs, sorry!

## Users and Teams

**TODO** - need to document user and team interfaces and visualizations
